...
export function RSVPModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const uploadTicket = async (file: File | undefined) => {
    ...
    setArrivalDate("");
    setArrivalTime("");
    setErrors((current) => ({ ...current, arrivalDate: undefined, arrivalTime: undefined }));
    try {
      let localExtraction: TicketDetails = {};
      try {
        localExtraction = await readTicketOnDevice(file);
      } catch {
        // Saving the ticket remains possible even when its text cannot be read.
      }
          ...
          <div className="rsvp-form-wrap">
            ...
            <form onSubmit={submit} noValidate>
              ...
              <div className={`form-field${errors.arrivalDate ? " has-error" : ""}`}>
                <label htmlFor="arrival-date">What date are you coming?</label>
                <input
                  id="arrival-date"
                  name="arrival-date"
                  type="date"
              ...
              <div className={`form-field${errors.arrivalTime ? " has-error" : ""}`}>
                <label htmlFor="arrival-time">What time are you arriving in NGP?</label>
                <input
                  id="arrival-time"
                  name="arrival-time"
                  type="time"
