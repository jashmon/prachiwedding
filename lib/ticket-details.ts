...
export function extractTicketDetails(rawText: string): TicketDetails {
  ...
  if (labelledArrival) {
    ...
    if (labelledArrival[4]) arrivalTime = toTwentyFourHourTime(labelledArrival[4], labelledArrival[5], labelledArrival[6]);
  }
  ...
  if (!arrivalDate) {
    if (isoDate) arrivalDate = `${isoDate[1]}-${isoDate[2].padStart(2, "0")}-${isoDate[3].padStart(2, "0")}`;
    else if (dayFirstDate) arrivalDate = toIsoDate(dayFirstDate[1], dayFirstDate[2], dayFirstDate[3]);
    else if (namedDate) arrivalDate = toIsoDate(namedDate[1], namedDate[2], namedDate[3]);
  }
  ...
  if (!arrivalTime && time) arrivalTime = toTwentyFourHourTime(time[1], time[2], time[3]);
